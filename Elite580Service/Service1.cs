    ...
    public partial class Service1 : ServiceBase
    {
        ...
        static void ResultsProcessing()
        {
                            ...
                            try
                            {
                                ...
                                if (MessageComment != "")
                                {
                                    AllMessage = AllMessage + "\r"
                                        + $"R|37|^^^0000^^^^{AnalyzerCode}|C|||N||F||^||20210409141558|{AnalyzerCode}"
            ...
            while (ServiceIsActive)
            {
                ...
                {
                    counter++;
                    ServiceLog(counter.ToString());
                    #region папки архива, результатов и ошибок

                    string OutFolder = System.Configuration.ConfigurationManager.AppSettings["FolderOut"];
                    // архивная папка
                    string ArchivePath = AnalyzerResultPath + @"\Archive";
                    // папка для ошибок
                    string ErrorPath = AnalyzerResultPath + @"\Error";
